...
describe('mergeExtractedProfile', () => {
  ...
  it('maps education rows with a separate id offset so ids never collide with experience', () => {
    const profile = makeProfile({
      experience: [{ role: 'Dev', company: 'Acme', start: '2020', end: '2022', description: '' }],
      education: [{ school: 'AAU', qualification: 'BSc CS', year: '2019' }],
    });
  ...
  it('ignores empty skill/experience arrays (no spurious AI badges)', () => {
    const { filled } = mergeExtractedProfile(emptyCv, makeProfile({ skills: [], experience: [] }));
    expect(filled.has('skills')).toBe(false);
...
describe('extractErrorMessage', () => {
  it.each([
    ...
    [422, "We couldn't read text from that file. Scanned or image-only resumes aren't supported."],
